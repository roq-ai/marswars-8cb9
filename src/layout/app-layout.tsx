...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['administrator', 'debater'];
  ...
  const { data, error, isLoading } = useSWR<OrganizationInterface[]>(
    ...
    () => getOrganizations({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    { name: 'Post', path: '/posts', entity: 'post', service: AccessServiceEnum.PROJECT, icon: FiFileText },
    { name: 'Video', path: '/videos', entity: 'video', service: AccessServiceEnum.PROJECT, icon: FiVideo },
